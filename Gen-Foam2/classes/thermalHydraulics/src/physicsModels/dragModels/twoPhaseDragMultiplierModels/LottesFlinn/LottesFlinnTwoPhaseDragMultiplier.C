...
#include "LottesFlinnTwoPhaseDragMultiplier.H"
...
Foam::scalar Foam::twoPhaseDragMultiplierModels::LottesFlinn::phi2
(
    const label& celli
) const
{
    if (onePhase(celli))
        return 1.0;
    return pow(max(mFluidPtr_->normalized()[celli], 1e-9),-exp_);
}
