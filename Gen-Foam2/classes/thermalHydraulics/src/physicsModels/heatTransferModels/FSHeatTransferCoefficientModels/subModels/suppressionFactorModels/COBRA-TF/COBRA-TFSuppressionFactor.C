...
#include "FSPair.H"
#include "COBRA-TFSuppressionFactor.H"
...
Foam::suppressionFactorModels::COBRA_TF::COBRA_TF
(
    const FSPair& pair,
...
Foam::scalar Foam::suppressionFactorModels::COBRA_TF::value
(
    const label& celli
) const
{
    const scalar& Ts(Tsat_[celli]);
    return min(max(Tf_[celli]-Ts, 0)/max(Twall_[celli]-Ts, 1e-3), 1.0);
}
