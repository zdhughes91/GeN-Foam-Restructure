...
void porousKEpsilon2PhaseCorrected<BasicTurbulenceModel>::correctNut()
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> porousKEpsilon2PhaseCorrected<BasicTurbulenceModel>::kSource() const
{
...
template<class BasicTurbulenceModel>
tmp<fvScalarMatrix> porousKEpsilon2PhaseCorrected<BasicTurbulenceModel>::epsilonSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
template<class BasicTurbulenceModel>
porousKEpsilon2PhaseCorrected<BasicTurbulenceModel>::porousKEpsilon2PhaseCorrected
(
...
:
    ...
    (
        this->subDict("porousKEpsilon2PhaseCorrectedProperties")
    ),
    convergenceLength_
    (
        IOobject
    ...
    (
        IOobject
        (
            IOobject::groupName("turbulenceIntensityAlphaCoeff", fluidName_),
            this->mesh_.time().timeName(),
            this->mesh_
        ),
        this->mesh_,
    ...
    Cmu3by4_(pow(Cmu_, 0.75))
{
    ...
    forAll(porousKEpsilon2PhaseCorrectedDict_.toc(), i)
    {
        ...
        forAll(regions, j)
        {
            ...
            scalar turbulenceIntensityAlphaCoeff
            (
                regionDict.get<scalar>("turbulenceIntensityAlphaCoeff")
            );
                ...
                if (!DhStructPtr_.valid())
                {
                    ...
                    (
                        ...
                        (
                            IOobject
                            (
                                "DhStructure.porousKEpsilon2PhaseCorrected",
                                this->mesh_.time().timeName(),
                                this->mesh_
                            ),
                            this->mesh_,
                ...
                if (!laminarReStructPtr_.valid())
                {
                    ...
                    (
                        ...
                        (
                            IOobject
                            (
                                "laminarReStructure.porousKEpsilon2PhaseCorrected",
                                this->mesh_.time().timeName(),
                                this->mesh_
                            ),
                            this->mesh_,
    ...
    //- Check that porousKEpsilon2PhaseCorrected properties were set for all porous regions
    forAll(structure_.regions(), i)
    {
        ...
        if (!found)
        {
            FatalErrorInFunction
                << "porousKEpsilon2PhaseCorrected region: " << region << " -> "
...
template<class BasicTurbulenceModel>
bool porousKEpsilon2PhaseCorrected<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void porousKEpsilon2PhaseCorrected<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
