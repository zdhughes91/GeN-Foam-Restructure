...
)
{
    ...
    */
    ...
    myOps::storePrevIterIfRelax(psi);
...
)
{
    ...
    (
        myMULEScontrols.lookupOrDefault<label>("nLimiterIter", 3)
    );

    const scalar smoothLimiter
    (
        myMULEScontrols.lookupOrDefault<scalar>("smoothLimiter", 0)
    );

    const scalar extremaCoeff
    (
        myMULEScontrols.lookupOrDefault<scalar>("extremaCoeff", 0)
    );
    ...
    scalarField psiMaxField(psiIf.size());
    scalarField psiMinField(psiIf.size());
    scalarField psiMaxn(psiIf.size());
    scalarField psiMinn(psiIf.size());
    forAll(psiMaxn, celli)
    {
        scalar m(psiMin[celli]);
        scalar M(psiMax[celli]);
        psiMaxField[celli] = M;
        psiMinField[celli] = m;
        psiMaxn[celli] = m;
        psiMinn[celli] = M;
    ...
    psiMaxn = min(psiMaxn + extremaCoeff*(psiMaxField - psiMinField), psiMaxField);
    psiMinn = max(psiMinn - extremaCoeff*(psiMaxField - psiMinField), psiMinField);

    if (smoothLimiter > Foam::SMALL)
    {
        psiMaxn =
            min(smoothLimiter*psiIf + (1.0 - smoothLimiter)*psiMaxn, psiMaxField);
        psiMinn =
            max(smoothLimiter*psiIf + (1.0 - smoothLimiter)*psiMinn, psiMinField);
    }
        ...
        forAll(sumlPhip, celli)
        {
            sumlPhip[celli] =
                ...
                (
                   ...
                   /(mSumPhim[celli] + Foam::ROOTVSMALL),
                    1.0), 0.0
                );
...
void Foam::myMULES::limitSum(SurfaceScalarFieldList& phiPsiCorrs)
{
    {
