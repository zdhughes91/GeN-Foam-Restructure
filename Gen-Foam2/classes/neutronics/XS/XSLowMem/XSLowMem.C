...
namespace Foam
{
    defineTypeNameAndDebug(XSLowMem, 0);
    defineRunTimeSelectionTable(XSLowMem, dictionary);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::XSLowMem::XSLowMem
(
    const fvMesh& mesh
)
:
...
{
    #include "readNuclearDataLowMem.H"
    #include "createXSfieldsLowMem.H"    
    init();
...
void Foam::XSLowMem::init()
{
    #include "setNeutronicsConstantsLowMem.H"
    #include "setPrecConstLowMem.H"
}

void Foam::XSLowMem::adjustDiscFactors(const PtrList<volScalarField>& fluxStar)
{
    if (adjustDiscFactors_)
    {
        #include "adjustDiscFactorsLowMem.H"
    }
}

Foam::tmp< Foam::volScalarField > Foam::XSLowMem::sigmaFromTo
(
    label momentI, 
    label energyJ, 
    label energyI,
